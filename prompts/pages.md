...
## P50 ロケ検索画面 (LocationSearchScreen)

| 項目 | 内容 |
|------|------|
| **ページ番号** | P50 |
| **ファイル** | `feature/inbound/.../locationsearch/LocationSearchScreen.kt` |
| **ルート** | `location_search` |
| **ヘッダー** | `{倉庫名} ロケ検索` |
| **主な機能** | 選択倉庫内の商品基本情報・在庫状況・ロケーション情報を検索表示 |
| **UI要素** | 検索入力、スキャン入力受付、複数商品候補リスト、商品情報、在庫ステータス、推奨ロケ、在庫ロケ一覧 |
| **ファンクションキー** | F1: 検索、F2: 戻る |
| **遷移先** | P03 メイン画面 |

### 表示仕様
- 検索条件は商品CD、商品名、JAN、社内JAN（`product_code` / `own_code`）。
- APIにはログインユーザーのデフォルト倉庫IDを `warehouse_id` として渡す。
- 複数商品が返った場合は候補リストを表示し、上下キーまたはタップで詳細対象を切り替える。
- 0件の場合は「該当商品なし」を表示する。
- `stock.status` は「在庫あり」「引当済」「在庫なし」「不明」として判別表示する。
- `locations.suggested` を推奨ロケとして表示し、`locations.stock` を在庫ロケ一覧として `available_quantity` 付きで表示する。
- 単価・税・原価・標準売価など価格系情報は表示しない。
